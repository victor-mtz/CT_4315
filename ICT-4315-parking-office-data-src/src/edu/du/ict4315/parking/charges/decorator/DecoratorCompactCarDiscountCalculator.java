...
public class DecoratorCompactCarDiscountCalculator extends ParkingChargeCalculatorDecorator {

  private String description = "Compact car 20% discount";
  private ParkingChargeCalculator calculator;
  
  public DecoratorCompactCarDiscountCalculator(ParkingChargeCalculator calculator) {
  ...
  public Money getParkingCharge(LocalDateTime in, LocalDateTime out, ParkingPermit p, ParkingLot lot) {
    ...
    if ( p != null && p.getCar() != null ) {
      if ( p.getCar().getType() == CarType.COMPACT ) {
        // 80% = 4/5
